...
**Status:** v1.0 — 2026-09-20 (Phase 4 complete except P4.5 — blocked(external): filing the upstream JAOT issue is the maintainer's GitHub account; Phase 5 complete: P5.1–P5.6 done, P5.7 gate **PASSED** — all four CI jobs green locally, evidence in `docs/ci.md`; P5.8 live e2e acceptance suite (`dev/e2e`, Playwright) — 11/11 cases green against the live stack, surfaced and fixed the manager ACL gaps (key storage, recipe validate, snapshot extraction) and added the apply/revert confirm dialogs; Phase 6: P6.1 `jaot_mrp` done — the second domain at the same quality bar, 14 tests green in the 5 620-test run; Phase 7 complete except P7.2 — blocked(external): the public store listing is the maintainer's call; P8.1 done: local tag `v1.0.0` + changelog. Remaining external items: P4.5, P7.2, any `git push` / announcement, first external user live)
...
| P5.8 | Live e2e acceptance suite (`dev/e2e`, Playwright + JSON-RPC against the `e2e` db of the dev stack + live JAOT): 11 cases — manager login; connection create/test (manager-gated key storage) + unreachable-endpoint failure; recipe Validate; MRP lifecycle (solve → baseline compare + delta → what-if rows → apply → revert, MO dates written + restored, audit log); infeasible (capacity cut to 80 → failed + infeasibility record); cancel from the UI; VRP lifecycle (apply/revert on pickings); staleness banner; viewer-role negatives (no manager buttons, no expression column); apply log list — 11/11 green (run 2026-09-20); screenshots in `dev/e2e/results/` (git-ignored); the API key is read from a file outside the repo. Found and fixed the manager ACL gaps (key storage/compute, recipe validate, snapshot extraction) and added the P3.5 apply/revert confirm dialogs | done | 2026-09-20 | 730a6c4 |
